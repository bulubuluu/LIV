...
def load_results(run_dir, run_name='carl'):
    ...
    candidate_logs_dirs = [
        os.path.join(carl_dir, 'lightning_logs'),
        os.path.join('.', 'lightning_logs'),
    ]

    logs_dir = None
    versions = None

    for d in candidate_logs_dirs:
        if os.path.isdir(d):
            found = [x for x in os.listdir(d) if re.match(r'version_\d+', x)]
            if found:
                logs_dir = d
                versions = found
                break
    ...
    if logs_dir is None:
        raise FileNotFoundError(f"No version folders found. Checked: {candidate_logs_dirs}")

    versions = sorted(versions, key=lambda v: int(re.search(r'\d+', v).group()), reverse=True)
    ...
    for version in versions:
        ...
        checkpoints = [
            ...
            if re.match(r'epoch=\d+-(val_loss|train_loss)=.+\.ckpt', f)
        ]
        if checkpoints:
            best_name = max(checkpoints, key=lambda f: int(re.search(r'epoch=(\d+)', f).group(1)))
            ckpt_path = os.path.join(checkpoint_dir, best_name)
            break
...
def get_likelihood_ratio(events, features, scaler_X, model):
    trainer = L.Trainer(accelerator='auto', devices=1, enable_progress_bar=False)
