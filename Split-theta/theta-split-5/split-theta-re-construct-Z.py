...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python3 split-theta-re-construct-Z.py <theta_tx> <theta_ty> <theta_xy> <theta_xz> <theta_yz>")
        sys.exit(1)
    ...
    input_file = os.path.join("result", theta_tag, "total.root")
    output_file = os.path.join("files-reconstructed", f"Reconstructed_Z1Z2_LHEF_{theta_tag}.root")
    tree_name = "LHEF"
    require_two_status2_Z = True

    os.makedirs("files-reconstructed", exist_ok=True)

    print(f"Using theta tag: {theta_tag}")
    print("Reading input ROOT file:")
    print(input_file)
    print("Writing output ROOT file:")
    print(output_file)
    print("DEBUG input_file =", input_file)
    print("DEBUG exists =", os.path.exists(input_file))
